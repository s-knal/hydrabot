...
test('pass, fail, error defaults will load when pull_request event is specified.', () => {
  ...
  expect(transformed.hydrabot[0].pass).toEqual(constants.DEFAULT_PR_PASS)
  expect(transformed.hydrabot[0].fail).toEqual(constants.DEFAULT_PR_FAIL)
  expect(transformed.hydrabot[0].error).toEqual(constants.DEFAULT_PR_ERROR)
})
...
test('pass, fail, error defaults will load when pull_request event is specified and user specified configs are not overridden.', () => {
  ...
  let transformed = V2Config.transform(yaml.safeLoad(config))
  expect(transformed.hydrabot[0].pass[0].payload.title).toBe('a title')
  expect(transformed.hydrabot[0].fail).toEqual(constants.DEFAULT_PR_FAIL)
  expect(transformed.hydrabot[0].error).toEqual(constants.DEFAULT_PR_ERROR)
  ...
  expect(transformed.hydrabot[0].pass[0].payload.title).toBe('a title')
  expect(transformed.hydrabot[0].fail[0].payload.title).toBe('a failed title')
  expect(transformed.hydrabot[0].error).toEqual(constants.DEFAULT_PR_ERROR)
  ...
  expect(transformed.hydrabot[0].pass[0].payload.title).toBe('a title')
  expect(transformed.hydrabot[0].fail[0].payload.title).toBe('a failed title')
  expect(transformed.hydrabot[0].error[0].payload.title).toBe('an errored title')
})
...
test('only pass, fail defaults ignore recipes that are not for pull_requests', () => {
  ...
  expect(transformed.hydrabot[0].pass).toEqual([])
  expect(transformed.hydrabot[0].fail).toEqual([])
  expect(transformed.hydrabot[0].error).toEqual([])
})
