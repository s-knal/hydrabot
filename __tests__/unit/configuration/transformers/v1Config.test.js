...
describe('Description no empty scenarios transformations', () => {
  const verify = (config) => {
    ...
    let dv = res.hydrabot[0].validate[0]

    expect(res.hydrabot[0].when).toBeDefined()
...
describe('Stale scenarios', () => {
  const loadWith = (moreConfig = '', expectedLength = 0) => {
    ...
    let res = v1Config.transform(yaml.safeLoad(config))
    let whenSchedule = res.hydrabot.filter(recipe => recipe.when === 'schedule.repository')
...
test('check that proper format is returned, including default pass, fail and error', async () => {
  ...
  let res = v1Config.transform(yaml.safeLoad(config))
  expect(res.hydrabot[0].when).toBeDefined()
  expect(res.hydrabot[0].pass).toBeDefined()
  expect(res.hydrabot[0].fail).toBeDefined()
  expect(res.hydrabot[0].error).toBeDefined()
})
...
test('checks that the content is tranformed correctly', async () => {
  ...
  `
  let res = v1Config.transform(yaml.safeLoad(config))
  const validate = res.hydrabot[0].validate
...
test('check all the simple config is transformed accurately', async () => {
  ...
  hydrabot:
    pull_requests:
      ...
      # Regular expression to be tested on the title. Not hydrabot when true.
      title: 'wip'

      # Only hydrabot when milestone is as specified below.
      milestone: 'version 1'

      # Only hydrabot when Project is as specified below
      project: 'Alpha'

      # exclude any of the hydrabot validation above. A comma separated list. For example, the following will exclude validations for approvals and label.
      exclude: 'title, label'
