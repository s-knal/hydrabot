...
module.exports = (issues, headDate, tailDate) => {
  ...
  data = data.filter((item) => {
    if (moment(item.created_at).isBetween(tailDate, headDate) && (item.user.login !== 'hydrab0t[bot]')) {
      return true
    } else {
      return false
    }
  ...
  if (data.length === 0) {
    ...
    data.reverse()
    // For Liked issue
    let likedIssue = data.filter((item) => item.reactions && (item.reactions['+1'] + item.reactions.laugh + item.reactions.hooray + item.reactions.heart > 0))
    ...
    // For Noisy issue
    let noisyIssue = data.filter((item) => item.comments && (item.comments > 0))
