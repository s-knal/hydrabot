...
module.exports = (pullRequests, headDate, tailDate) => {
  ...
  data = data.filter((item) => {
    if (moment(item.created_at).isBetween(tailDate, headDate) && item.state === 'open' && !item.merged_at && moment(item.created_at).isSame(item.updated_at) && item.user.login !== 'hydrab0t[bot]') {
      return true
    }
    if (moment(item.updated_at).isBetween(tailDate, headDate) && item.state === 'open' && !item.merged_at && item.user.login !== 'hydrab0t[bot]') {
      return true
    }
    if (moment(item.merged_at).isBetween(tailDate, headDate) && item.state === 'closed' && item.user.login !== 'hydrab0t[bot]') {
      return true
    }
  })
  if (data.length === 0) {
